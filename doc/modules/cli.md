...
- **Plugin Architecture**: Based on the `entry points` mechanism, automatically discovers installed `bio-analyze-*` modules (e.g., `bio-analyze-docking`, `bio-analyze-plot`).
...
Once the corresponding functional module package is installed, it can be called directly via `bioanalyze`:
