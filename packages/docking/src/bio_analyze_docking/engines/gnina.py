...
class GninaEngine(BaseDockingEngine):
    """
    Gnina-based docking engine implementation.

    Gnina is a fork of Smina that integrates deep learning scoring functions (CNN scoring).
    This engine invokes the gnina command-line tool via subprocess.
    """
    ...
    def __init__(self, receptor: Path, ligand: Path, output_dir: Path):
        """
        Initialize the Gnina docking engine.
        """
    ...
    def dock(self, exhaustiveness: int = 8, n_poses: int = 9, min_rmsd: float = 1.0, timeout: float = 3600):
        ...
        try:
            ...
            raise RuntimeError(f"Gnina 对接失败: {e.stderr}") from e
        except Exception as e:
            logger.error(f"Gnina 执行异常: {e}")
            raise RuntimeError(f"Gnina 对接异常: {e}") from e
    ...
    def get_all_poses_info(self, n_poses: int = 9) -> list[dict[str, Any]]:
        """
        Return information for all poses: energy, RMSD lower bound, RMSD upper bound, and CNN score.
