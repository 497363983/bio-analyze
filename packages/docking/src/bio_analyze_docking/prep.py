...
def prepare_ligand(input_file: Path, output_file: Path, add_hydrogens: bool = True) -> Path:
    """
    Prepare ligand for docking (SDF/SMILES/CIF -> PDBQT).
    ...
    Args:
        input_file (Path):
            Path to input ligand file (SDF, MOL2, PDB, SMI, CIF).
        output_file (Path):
...
def prepare_receptor(
    ...
    Args:
        ...
        output_file (Path):
            Path to output PDBQT file.
        add_hydrogens (bool, optional):
            Whether to add hydrogens. Defaults to True.
        ph (float, optional):
            pH value for protonation state calculation. Defaults to 7.4.
        ...
        charge_model (str, optional):
            Charge model for Meeko (e.g. 'gasteiger', 'zero', 'scikit_learn'). Defaults to 'gasteiger'.
    ...
    try:
        ...
        with open(fixed_pdb_path, "w", encoding="utf-8") as f:
    ...
    def run_openbabel_fallback(error_msg: str) -> bool:
        ...
        try:
            ...
            if openbabel:
                ...
                if not ob_conversion.SetInAndOutFormats("pdb", "pdbqt"):
                    raise RuntimeError("Failed to set OpenBabel formats")

                mol = openbabel.OBMol()

                if not ob_conversion.ReadFile(mol, str(processing_file)):
                    raise RuntimeError(f"OpenBabel failed to read {processing_file}")

                # -xr: 刚性分子
                ob_conversion.AddOption("r", openbabel.OBConversion.OUTOPTIONS)

                # 如果需要计算电荷（Gasteiger 通常是默认值）
                if charge_model == "gasteiger":
                    mol.AddHydrogens()  # 确保识别氢

                if not ob_conversion.WriteFile(mol, str(output_file)):
    ...
    try:
        ...
        if not run_openbabel_fallback(f"Exit code {e.returncode}: {e.stderr}"):
            raise RuntimeError(f"Meeko CLI failed for {input_file.name}: {e.stderr}") from e
        ...
        if not run_openbabel_fallback(str(e)):
            raise RuntimeError(f"Meeko failed ({e}) AND OpenBabel fallback failed") from e
...
def get_box_from_ligand(ligand_file: Path, padding: float = 4.0) -> tuple[list[float], list[float]]:
    """
    Calculate grid box center and size from reference ligand.

    Args:
        ligand_file (Path):
            Path to ligand file.
        padding (float, optional):
            Extra padding (Angstrom). Defaults to 4.0.
    ...
    Raises:
        FileNotFoundError:
            If ligand file does not exist.
        ValueError:
            If ligand coordinates cannot be read.
    """
...
def get_box_from_receptor(receptor_file: Path, padding: float = 0.0) -> tuple[list[float], list[float]]:
    """
    Calculate bounding box center and size from receptor.

    Args:
        receptor_file (Path):
            Path to receptor file (PDB/PDBQT).
        padding (float, optional):
            Extra padding (Angstrom). Defaults to 0.0.
    ...
    Raises:
        FileNotFoundError:
            If receptor file does not exist.
        ValueError:
            If receptor coordinates cannot be parsed.
    """
