...
from .genome import GenomeManager, discover_reference_files
...
def _recover_reference_info(context: Context, ref_info: dict | None, logger, stage: str) -> dict:
    """Recover reference FASTA/GTF information from pipeline context and reference output."""
    recovered = dict(ref_info or {})
    if recovered.get("fasta") and recovered.get("gtf"):
        return recovered

    ref_dir = context.output_dir / "reference"
    if ref_dir.exists():
        genome_query = context.get("assembly") or context.get("species")
        localname = context.get("assembly")
        discovered = discover_reference_files(
            ref_dir,
            genome_query=str(genome_query) if genome_query else None,
            localname=str(localname) if localname else None,
        )
        if discovered:
            if discovered.get("fasta") and not recovered.get("fasta"):
                recovered["fasta"] = discovered["fasta"]
            if discovered.get("gtf") and not recovered.get("gtf"):
                recovered["gtf"] = discovered["gtf"]

    if context.get("genome_fasta") and not recovered.get("fasta"):
        recovered["fasta"] = context.genome_fasta
    if context.get("genome_gtf") and not recovered.get("gtf"):
        recovered["gtf"] = context.genome_gtf

    if recovered:
        context.ref_info = recovered
    else:
        logger.warning("Reference information could not be recovered for %s.", stage)
    return recovered


class CheckDependenciesNode(Node):
    """
    Node to check for external dependency tools.
    """
    ...
    def run(self, context: Context, progress: Progress, logger):
        ...
        if missing_tools:
            msg = (
                f"Missing required external tools for step '{step or 'all'}': "
                f"{', '.join(missing_tools)}. Please install them and ensure they are in your PATH."
            )
            logger.error(msg)
...
class SRADownloadNode(Node):
    """
    SRA data download node.
    """

    def run(self, context: Context, progress: Progress, logger):
        """
        Execute SRA data download.
        """
...
class GenomePrepNode(Node):
    """
    Genome preparation node.
    """

    def run(self, context: Context, progress: Progress, logger):
        """
        Execute genome preparation.
        """
...
class QCNode(Node):
    ...
    def run(self, context: Context, progress: Progress, logger):
        """
        Execute quality control and trimming.
        """
...
class StarAlignNode(Node):
    ...
    def run(self, context: Context, progress: Progress, logger):
        ...
        ref_info = _recover_reference_info(context, ref_info, logger, "STAR alignment")
...
class QuantNode(Node):
    ...
    def run(self, context: Context, progress: Progress, logger):
        ...
        # Recover ref_info
        ref_info = _recover_reference_info(context, ref_info, logger, "quantification")
        ...
        if clean_reads:
            quant_mgr = QuantManager(
                ...
                threads=context.threads,
                tool=context.get("quant_tool", "salmon"),
                compare_tools=context.get("quant_compare_tools", []),
                tool_config=context.get("quant_config", {}),
            )
...
class DENode(Node):
    """
    Differential expression analysis node.
    """
