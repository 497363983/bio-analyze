...
class SRAManager:
    ...
    def __init__(self, output_dir: Path, threads: int = 4):
        """
        Initialize the SRA data manager.
    ...
    def check_dependencies(self):
        ...
        Raises:
            RuntimeError: If `prefetch` or `fasterq-dump` is not found.
        """
    ...
    def download(self, sra_ids: list[str]) -> Path:
        """
        Download SRA data and convert it to FASTQ.

        Args:
            sra_ids (list[str]): List of SRA IDs.

        Returns:
            Path: Directory containing downloaded FASTQ files.
        """
    ...
    def process_single_sra(self, sra_id: str):
        """
        Process a single SRA ID through `prefetch` and `fasterq-dump`.
    ...
    def _compress_fastq(self, sra_id: str):
        """Compress generated .fastq files to .fastq.gz.  Args: sra_id (str): SRA ID. """
