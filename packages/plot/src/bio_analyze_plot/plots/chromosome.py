...
class ChromosomePlot(BasePlot):
    """
    Chromosome Reads Coverage Distribution Plot (Track style).
    """
    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        """
        Plot Reads coverage distribution on chromosomes (differentiating strands).

        Args:
            data:
                DataFrame containing bin statistics. Required columns: chrom, pos (bin start), counts_pos, counts_neg.
            chrom_col:
                Column name for chromosome.
            pos_col:
                Column name for position (bin start).
            pos_counts_col:
                Column name for positive strand reads count.
            neg_counts_col:
                Column name for negative strand reads count.
            title:
                Chart title.
            output:
                Path to save the chart.
            max_chroms:
                Maximum number of chromosomes to display (after sorting by length or name).
            bin_size:
                Bin size (used for X-axis unit conversion or annotation).
            color_pos:
                Color for positive strand.
            color_neg:
                Color for negative strand.
            **kwargs:
                Other plotting parameters, such as label_pos, label_neg, zero_line_kws, etc.
        """
        # 获取主题特定参数
