    ...
    def plot(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            ylabel: Y轴标签。
            output: 保存图表的路径。
            error_bar_type: 误差棒类型 (SD, SE, CI)。
            error_bar_ci: 当类型为 CI 时的置信区间大小 (默认 95)。
            error_bar_max: 误差棒上限的列名 (暂不支持手动绘制，仅占位)。
            error_bar_min: 误差棒下限的列名 (暂不支持手动绘制，仅占位)。
            error_bar_capsize: 误差棒的横线宽度 (单位: points)。
            err_color: 误差棒颜色。
            cap_color: 误差棒横线颜色 (对于 lineplot，通常与 err_color 一致)。
            markers: 是否显示数据点标记，或指定标记样式。
            dashes: 是否显示虚线，或指定虚线样式。
            smooth: 是否启用平滑曲线拟合。
            smooth_points: 平滑曲线的插值点数。
        """
        # 获取主题特定参数
